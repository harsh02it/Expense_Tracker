...
router.get("/", async (req, res) => {
  const totalInventory = await TotalInventory.find();
  res.send(totalInventory);
});

router.post("/", async (req, res) => {
  const { error } = validateTotalInventory(req.body);
  ...
  let totalInventory = new TotalInventory({
    product: {
        _id: totalInventory._id
    },
    quantity: req.body.quantity,
  });

  totalInventory = await totalInventory.save();
...
function validateTotalInventory(totalInventory) {
  const schema = {
    _id: Joi.string().required(),
    quantity: Joi.number().min(0).required(),
  };

  return Joi.validate(totalInventory, schema);
}

module.exports = router;
module.exports.TotalInventory = TotalInventory;
